...
class Memory():
    ...
    def update_counter(self, counter_type, type):
        if counter_type == 'global':
            if type == Data_types['INTEGER']:
                self.count_global_int = self.count_global_int + 1
            elif type == Data_types['FLOAT']:
                self.count_global_float = self.count_global_float + 1
            elif type == Data_types['CHARACTER']:
                self.count_global_char = self.count_global_char + 1
            elif type == Data_types['BOOLEAN']:
                self.count_global_bool = self.count_global_bool + 1
        elif counter_type == 'local':
            if type == Data_types['INTEGER']:
                self.count_local_int = self.count_local_int + 1
            elif type == Data_types['FLOAT']:
                self.count_local_float = self.count_local_float + 1
            elif type == Data_types['CHARACTER']:
                self.count_local_char = self.count_local_char + 1
            elif type == Data_types['BOOLEAN']:
                self.count_local_bool = self.count_local_bool + 1
        elif counter_type == 'temp':
            if type == Data_types['INTEGER']:
                self.count_temp_int = self.count_temp_int + 1
            elif type == Data_types['FLOAT']:
                self.count_temp_float = self.count_temp_float + 1
            elif type == Data_types['CHARACTER']:
                self.count_temp_char = self.count_temp_char + 1
            elif type == Data_types['BOOLEAN']:
                self.count_temp_bool = self.count_temp_bool + 1
        elif counter_type == 'const':
            if type == Data_types['INTEGER']:
                self.count_const_int = self.count_const_int + 1
            elif type == Data_types['FLOAT']:
                self.count_const_float = self.count_const_float + 1
            elif type == Data_types['CHARACTER']:
                self.count_const_char = self.count_const_char + 1
            elif type == Data_types['BOOLEAN']:
